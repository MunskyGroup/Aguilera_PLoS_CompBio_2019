...
%% The code reproduces figure S1 in the publication.
...
evaluatingFRAP =0;
k_elongationMean_vector = 10;
k_initiation_vector = [0.03,0.3,0.06];
usingSynteticGene =0;
geneLength =1000;
...
for j =1:length(k_initiation_vector)
 nameplot =strrep(['_',num2str(k_initiation_vector(j))],'.','_'); 
 plotCollisionDistributions_ki (collisions{j},nameplot,folderName)
 max (collisions{j})
end

%% Plotting Kymographs 
for j =1:length(k_initiation_vector)
 nameplot =strrep(['kymograph_',num2str(k_initiation_vector(j))],'.','_');
